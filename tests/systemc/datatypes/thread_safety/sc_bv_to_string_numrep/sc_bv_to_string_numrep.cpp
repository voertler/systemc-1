...
std::size_t run_workload()
{
    ...
    {
        sc_dt::sc_bv<32> varth("10100101101001011010010110100101");
        std::size_t total = 0;
    ...
    {
        sc_dt::sc_bv<32> varm("01011010010110100101101001011010");
        std::size_t total = 0;
        for (int i = 0; i < NUMRUNS; ++i)
...
{
    std::cout << "test sc_bv_to_string_numrep started" << std::endl;
    ...
    for (int i = 0; i < NUMTHREADS; ++i)
        my_thread[i].join();
    std::cout << "test sc_bv_to_string_numrep finished" << std::endl;
    return 0;
}
