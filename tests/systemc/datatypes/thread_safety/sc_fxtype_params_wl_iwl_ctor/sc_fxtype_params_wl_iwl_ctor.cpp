...
std::size_t run_workload()
{
...

    {
        ...
        for (int i = 0; i < NUMRUNS; ++i) {
            sc_dt::sc_fxtype_params varth(8, 4);
...

    {
        std::size_t total = 0;
        for (int i = 0; i < NUMRUNS; ++i) {
            sc_dt::sc_fxtype_params varm(16, 8);
...
{
    std::cout << "test sc_fxtype_params_wl_iwl_ctor started" << std::endl;
    ...
    for (int i = 0; i < NUMTHREADS; ++i)
        my_thread[i].join();
    std::cout << "test sc_fxtype_params_wl_iwl_ctor finished" << std::endl;
    return 0;
}
