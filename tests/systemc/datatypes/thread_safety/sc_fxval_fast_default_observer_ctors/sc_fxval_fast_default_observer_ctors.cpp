...
std::size_t run_workload()
{
...

    {
        ...
        for (int i = 0; i < NUMRUNS; ++i) {
            sc_dt::sc_fxval_fast varth;
            sc_dt::sc_fxval_fast tmp1(1.25);
            sc_dt::sc_fxval_fast tmp2("3.75");
            sc_dt::sc_int<8> base = 7;
            sc_dt::sc_fxval_fast tmp3(base);
...

    {
        std::size_t total = 0;
        for (int i = 0; i < NUMRUNS; ++i) {
            sc_dt::sc_fxval_fast varm(2);
            sc_dt::sc_fxval_fast tmp1(2.5);
            sc_dt::sc_fxval_fast tmp2("4.5");
...
{
    std::cout << "test sc_fxval_fast_default_observer_ctors started" << std::endl;
    ...
    for (int i = 0; i < NUMTHREADS; ++i)
        my_thread[i].join();
    std::cout << "test sc_fxval_fast_default_observer_ctors finished" << std::endl;
    return 0;
}
