...
std::size_t run_workload()
{
...

    {
        ...
        for (int i = 0; i < NUMRUNS; ++i)
            total += static_cast<unsigned>(varth.range(7, 0).to_int() & 0xff);
...

    {
        ...
        std::size_t total = 0;
        for (int i = 0; i < NUMRUNS; ++i)
            total += static_cast<unsigned>(varm.range(15, 8).to_int() & 0xff);
        combined_total += total;
    }
...
{
    std::cout << "test sc_signed_range started" << std::endl;
    ...
    for (int i = 0; i < NUMTHREADS; ++i)
        my_thread[i].join();
    std::cout << "test sc_signed_range finished" << std::endl;
    return 0;
}
