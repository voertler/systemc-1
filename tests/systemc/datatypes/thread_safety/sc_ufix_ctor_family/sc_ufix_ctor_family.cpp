...
std::size_t run_workload()
{
...

    {
        ...
        for (int i = 0; i < NUMRUNS; ++i) {
            sc_dt::sc_ufix varth;
            sc_dt::sc_ufix tmp1(8, 4);
            sc_dt::sc_ufix tmp2(sc_dt::SC_TRN, sc_dt::SC_WRAP);
            sc_dt::sc_ufix tmp3(16, 8, sc_dt::SC_TRN_ZERO, sc_dt::SC_SAT, 1);
            sc_dt::sc_fxcast_switch cast_switch;
            sc_dt::sc_ufix tmp4(cast_switch);
            sc_dt::sc_ufix tmp5(sc_dt::sc_fxtype_params(16, 8));
...

    {
        std::size_t total = 0;
        for (int i = 0; i < NUMRUNS; ++i) {
            sc_dt::sc_ufix varm(1.25);
            sc_dt::sc_ufix tmp1("3.5");
            sc_dt::sc_int<8> base = 7;
            sc_dt::sc_ufix tmp2(base);
            sc_dt::sc_uint<8> ubase = 9;
            sc_dt::sc_ufix tmp3(ubase);
...
{
    std::cout << "test sc_ufix_ctor_family started" << std::endl;
    ...
    for (int i = 0; i < NUMTHREADS; ++i)
        my_thread[i].join();
    std::cout << "test sc_ufix_ctor_family finished" << std::endl;
    return 0;
}
