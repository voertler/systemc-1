...
std::size_t run_workload()
{
...

    {
        ...
        for (int i = 0; i < NUMRUNS; ++i) {
            sc_dt::sc_ufixed_fast<16, 8> varth;
            sc_dt::sc_fxcast_switch cast_switch;
            sc_dt::sc_ufixed_fast<16, 8> tmp1(cast_switch);
            sc_dt::sc_ufixed_fast<16, 8> tmp2(1.25);
...

    {
        ...
        for (int i = 0; i < NUMRUNS; ++i) {
            ...
            sc_dt::sc_ufixed_fast<16, 8> varm(base);
            sc_dt::sc_fxcast_switch cast_switch;
            sc_dt::sc_ufixed_fast<16, 8> tmp1(2.5, cast_switch);
...
{
    std::cout << "test sc_ufixed_fast_ctor_family started" << std::endl;
    ...
    for (int i = 0; i < NUMTHREADS; ++i)
        my_thread[i].join();
    std::cout << "test sc_ufixed_fast_ctor_family finished" << std::endl;
    return 0;
}
