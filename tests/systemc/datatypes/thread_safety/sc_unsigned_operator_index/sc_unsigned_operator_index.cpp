...
std::size_t run_workload()
{
...

    {
        ...
        for (int i = 0; i < NUMRUNS; ++i)
            total += static_cast<unsigned>(varth[i & 7]);
...

    {
        ...
        std::size_t total = 0;
        for (int i = 0; i < NUMRUNS; ++i)
            total += static_cast<unsigned>(varm[(i + 1) & 7]);
        combined_total += total;
    }
...
{
    std::cout << "test sc_unsigned_operator_index started" << std::endl;
    ...
    for (int i = 0; i < NUMTHREADS; ++i)
        my_thread[i].join();
    std::cout << "test sc_unsigned_operator_index finished" << std::endl;
    return 0;
}
