...
std::size_t run_workload()
{
...

    {
        ...
        for (int i = 0; i < NUMRUNS; ++i)
            total += static_cast<unsigned>(varth(7, 0).to_uint() & 0xff);
...

    {
        ...
        std::size_t total = 0;
        for (int i = 0; i < NUMRUNS; ++i)
            total += static_cast<unsigned>(varm(15, 8).to_uint() & 0xff);
        combined_total += total;
    }
...
{
    std::cout << "test sc_unsigned_operator_paren started" << std::endl;
    ...
    for (int i = 0; i < NUMTHREADS; ++i)
        my_thread[i].join();
    std::cout << "test sc_unsigned_operator_paren finished" << std::endl;
    return 0;
}
